...
class Bookings {
    ...
    init(eventKey: String, eventData: Dictionary<String, AnyObject>) {
        ...
        if let recipePrice = eventData["Eventprice"] as? String {
            _recipePrice = recipePrice
        }
        ...
        if let latitudeLocation = eventData["Eventlatitude"] as? Double {
            _latitudeLocation = latitudeLocation
        }
        
        if let longitudeLocation = eventData["Eventlongitude"] as? Double {
            _longitudeLocation = longitudeLocation
        }
        ...
        _eventRef = Database.database().reference().child("booking").child(_eventKey)
    }
}
